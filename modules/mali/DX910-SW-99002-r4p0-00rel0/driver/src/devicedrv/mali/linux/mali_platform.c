...
static DEVICE_ATTR(mali_vol, 0444, mali_vol_show, NULL);
...
extern void mali_dev_pause(void);
...
int sun7i_mali_platform_device_register(void)
{
    ...
    if (0 == err){
       // mali_gpu_data.dedicated_mem_start = gpu_addr.paddr;
        ...
        MALI_DEBUG_PRINT(0,("sun7i_mali_platform_device_register() add data failed!\n"));
...
static void mali_dvfs_work_handler(struct work_struct *w)
{
...
#ifdef CONFIG_DYNAMIC_GPU_FREQ
    {
        ...
        if (mali_dvfs_utilization > 240){
            mali_freq = mali_dvfs_table[mode].freq_max;
        }else if (mode == SW_POWERNOW_PERFORMANCE && mali_dvfs_utilization < 210  && mali_dvfs_utilization >150 ){
            mali_freq = 240*1000*1000;
        }else if (mali_dvfs_utilization < 150){
...
static struct notifier_block mali_pm_notifier = {
    .notifier_call = mali_pm_notify,
//    .priority = 0xFF,
};
