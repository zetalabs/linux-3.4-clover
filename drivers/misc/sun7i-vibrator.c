/* Vibrator driver for sun7i platform 
...
//
//static script_item_u motor_buzzer;
//
enum {
	DEBUG_INIT = 1U << 0,
...
static void set_sun7i_vibrator(int on)
{
	dprintk(DEBUG_DATA_INFO, "sun7i_vibrator on %d\n", on);
	if(on) {
		__gpio_set_value(vibe_gpio_hdle.val.gpio.gpio, !vibe_off);
		//__gpio_set_value(motor_buzzer.gpio.gpio, !vibe_off);
...
static void vibrator_enable(struct timed_output_dev *dev, int value)
{
	...
	dprintk(DEBUG_DATA_INFO, "sun7i_vibrator enable %d\n", value);
...
static enum hrtimer_restart vibrator_timer_func(struct hrtimer *timer)
{
	vibe_state = 0;
	schedule_work(&vibrator_work);
	dprintk(DEBUG_DATA_INFO, "sun7i_vibrator timer expired\n");
	return HRTIMER_NORESTART;
}

static struct timed_output_dev sun7i_vibrator = {
	.name = "vibrator",
	.get_time = vibrator_get_time,
	.enable = vibrator_enable,
};

static int __init sun7i_vibrator_init(void)
{
	int vibe_used;
	script_item_u	val;
	script_item_value_type_e  type;

	dprintk(DEBUG_INIT, "hello, sun7i_vibrator init\n");
	...
	//
	//	script_get_item("motor_para", "motor_buzzer", &motor_buzzer);
	//
	...
	//
#if 0	
	if(0 != gpio_request(motor_buzzer.gpio.gpio, NULL)) {
		printk(KERN_ERR "ERROR: vibe Gpio_request is failed\n");
	}
	
	if (0 != sw_gpio_setall_range(&motor_buzzer.gpio, 1)) {
		printk(KERN_ERR "vibe gpio set err!");
		goto exit;
	}
#endif	
	...
	timed_output_dev_register(&sun7i_vibrator);

	dprintk(DEBUG_INIT, "sun7i_vibrator init end\n");
...
static void __exit sun7i_vibrator_exit(void)
{
	dprintk(DEBUG_INIT, "bye, sun7i_vibrator_exit\n");
	timed_output_dev_unregister(&sun7i_vibrator);
	gpio_free(vibe_gpio_hdle.val.gpio.gpio);	
}
module_init(sun7i_vibrator_init);
module_exit(sun7i_vibrator_exit);

/* Module information */
MODULE_DESCRIPTION("timed output vibrator device for sun7i");
MODULE_LICENSE("GPL");
