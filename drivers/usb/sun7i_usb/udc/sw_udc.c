...
void udc_timer_func(unsigned long arg)
{
    ...
    DMSG_INFO("sw_powernow_set_usb SW_POWERNOW_USBSTAT_INACTIVE\n");
    sw_powernow_set_usb(SW_POWERNOW_USBSTAT_INACTIVE);
}

static void udc_powernow_switch(struct work_struct *work)
{
	DMSG_INFO("sw_powernow_set_usb SW_POWERNOW_USBSTAT_ACTIVE\n");
    sw_powernow_set_usb(SW_POWERNOW_USBSTAT_ACTIVE);
}
#endif
...
*******************************************************************************
*/
int dma_write_fifo(struct sw_udc_ep *ep, struct sw_udc_request *req)
{
	u32   	left_len 	= 0;
	u32		idx         = 0;
	int		fifo_reg    = 0;
...
static int sw_udc_write_fifo(struct sw_udc_ep *ep, struct sw_udc_request *req)
{
	...
	if(req->req.length > ep->ep.maxpacket){
		if (!dev->udc_actived) {
			schedule_work(&dev->udc_active_work);
			dev->udc_actived = 1;
		}
...
void sw_udc_dma_completion(struct sw_udc *dev, struct sw_udc_ep *ep, struct sw_udc_request *req)
{
	...
	if(dma_transmit_len < req->req.length){
		DMSG_DBG_UDC("WRN: DMA recieve data not complete, (%d, %d, %d)\n",
...
#ifdef CONFIG_UDC_ACTIVE
	del_timer(&udc->udc_active_timer);
	cancel_work_sync(&udc->udc_active_work);
#endif
	return 0;
}
