...
//add by xgc	
	...
	type = script_get_item("lcd0_para", "sys_power_ctrl_used", &val1);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type)
	{
		printk("%s script_get_item sys_power_ctrl_used = %d\n",__FUNCTION__, val1.val);
	}
	
	sys_power_ctrl_used = val1.val;

	if(!sys_power_ctrl_used) 
	{
	...
	{	
		...
		{
			printk("sys_power_ctrl type err ! \n");
			gpio_free(val1.gpio.gpio);
		}
		
		if(0 != gpio_request(val1.gpio.gpio, NULL)) 
		{
			printk("ERROR: sys_power_ctrl Gpio_request is failed !\n");		
		}	
		
		if (0 != sw_gpio_setall_range(&val1.gpio, 1)) 
		{
