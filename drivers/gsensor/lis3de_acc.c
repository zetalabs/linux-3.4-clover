...
static int gsensor_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	...
	if (config_info.twi_id == adapter->nr) {
		...
		dprintk(DEBUG_INIT, "%s:lis3de_acc Device not found, \
...
static struct attribute* lis3de_attrs[] = {
	&dev_attr_enable.attr,
	&dev_attr_delay.attr,
	NULL
};

static const struct attribute_group lis3de_group =
{
	.attrs = lis3de_attrs,
};
...
static int lis3de_acc_input_init(struct lis3de_acc_status *stat)
{
	...
	err = sysfs_create_group(&stat->input_dev->dev.kobj, &lis3de_group);
	if (err) {
...
static int __devexit lis3de_acc_remove(struct i2c_client *client)
{
	...
	if (stat->pdata->gpio_int2 >= 0) {
        ...
        sysfs_remove_group(&stat->input_dev->dev.kobj, &lis3de_group);
	lis3de_acc_input_cleanup(stat);
	lis3de_acc_device_power_off(stat);
	remove_sysfs_interfaces(&client->dev);
