...
static irqreturn_t lis3dh_acc_isr1(int irq, void *dev)
{
	...
	dprintk(DEBUG_INIT, "%s: isr1 queued\n", LIS3DH_ACC_DEV_NAME);
...
static irqreturn_t lis3dh_acc_isr2(int irq, void *dev)
{
	...
	dprintk(DEBUG_INIT, "%s: isr2 queued\n", LIS3DH_ACC_DEV_NAME);
...
static void lis3dh_acc_irq1_work_func(struct work_struct *work)
{
	...
	;
	/*  */
	dprintk(DEBUG_INIT, "%s: IRQ1 triggered\n", LIS3DH_ACC_DEV_NAME);
//exit:
	enable_irq(acc->irq1);
}
...
static void lis3dh_acc_irq2_work_func(struct work_struct *work)
{
	...
	dprintk(DEBUG_INIT, "%s: IRQ2 triggered\n", LIS3DH_ACC_DEV_NAME);
//exit:
	enable_irq(acc->irq2);
}
				     ...
				     const char *buf, size_t size)
{
	...
	if (!interval_ms)
		return -EINVAL;
	interval_ms = min((unsigned int)interval_ms,acc->pdata->min_interval);
	mutex_lock(&acc->lock);
...
static int lis3dh_acc_input_init(struct lis3dh_acc_data *acc)
{
	...
	err = sysfs_create_group(&acc->input_dev->dev.kobj, &lis3dh_group);
	if (err) {
...
static int __devexit lis3dh_acc_remove(struct i2c_client *client)
{
	...
	if(acc->pdata->gpio_int2 >= 0){
        ...
        sysfs_remove_group(&acc->input_dev->dev.kobj, &lis3dh_group);
	lis3dh_acc_input_cleanup(acc);
	lis3dh_acc_device_power_off(acc);
	remove_sysfs_interfaces(&client->dev);
