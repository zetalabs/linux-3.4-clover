...
__s32 DE_SCAL_Config_3D_Src(__u8 sel, __scal_buf_addr_t *addr, __scal_src_size_t *size,
                           __scal_src_type_t *type, __scal_3d_inmode_t trdinmode, __scal_buf_addr_t *addrtrd, __s32 parallax_para)
{
    __u8 w_shift, h_shift;
    ...
    __u32 parallax_para_l, parallax_para_r;

    /*
    parallax_para > 0,the left image move to the left
    parallax_para < 0,the right image move to the left
    */
    if (parallax_para >= 0)
    {
        parallax_para_l = parallax_para * PARALLAX_STEP;
        parallax_para_r = 0;
    }
    else
    {
        parallax_para_l = 0;
        parallax_para_r = (-1) * parallax_para * PARALLAX_STEP;
    }
    ...
    in_w0 -= parallax_para_l + parallax_para_r;
    in_w1 -= parallax_para_l + parallax_para_r;
	...
	else if(((trdinmode == DE_SCAL_3DIN_SSF) ||(trdinmode == DE_SCAL_3DIN_SSH)) && (type->mod == DE_SCAL_UVCOMBINEDMB))
	{
		...
		//block offset
		de_scal_ch0_dx0 = ((x_off0 + parallax_para_l) & 0x1f);
		de_scal_ch0_dy0 = (y_off0 & 0x1f);
		de_scal_ch0_dx1 = ((de_scal_ch0_dx0 & 0x1f) + in_w0 + 0x1f) &0x1f;
		de_scal_ch1_dx0 = (((x_off1 + parallax_para_l)<<1)&0x1f);
		de_scal_ch1_dy0 = ((y_off1)&0x1f);
		de_scal_ch1_dx1 = ((de_scal_ch1_dx0 & 0x1e) + (in_w1<<1) + 0x1f) & 0x1f; 
		...
		scal_dev[sel]->mb_off1.bits.y_offset0 = de_scal_ch1_dy0;
		scal_dev[sel]->mb_off1.bits.x_offset1 = de_scal_ch1_dx1; 
		scal_dev[sel]->trd_mb_off0.bits.x_offset0 = (image_w0 + x_off0 + parallax_para_r) & 0x1f;
		scal_dev[sel]->trd_mb_off0.bits.y_offset0 = de_scal_ch0_dy0;
		scal_dev[sel]->trd_mb_off0.bits.x_offset1 = (((image_w0 + x_off0 + parallax_para_r) & 0x1f) + in_w0 + 0x1f)&0x1f;
		scal_dev[sel]->trd_mb_off1.bits.x_offset0 = ((2 * (image_w1 + x_off1 + parallax_para_r)) & 0x1f);
		scal_dev[sel]->trd_mb_off1.bits.y_offset0 = de_scal_ch1_dy0;
		scal_dev[sel]->trd_mb_off1.bits.x_offset1 = (((2 * (image_w1 + x_off1 + parallax_para_r)) & 0x1f) + (in_w1<<1) + 0x1f)&0x1f;

		de_scal_ch0_offset = ((2 *image_w0 + 0x1f) &0xffe0) * (y_off0& 0xffe0) + ((y_off0& 0x01f)<<5) + 
                              (((x_off0 + parallax_para_l)& 0xffe0)<<5);
        de_scal_ch1_offset = ((((2 * image_w1)<< 0x01)+0x1f)&0xffe0) * ((y_off1) & 0xffe0) + 
                              (((y_off1) & 0x01f)<<5) + ((((x_off1 + parallax_para_l)<<0x01) & 0xffe0)<<5);
        de_scal_ch2_offset = 0x0;

		de_scal_ch0r_offset = ((2 *image_w0 + 0x1f) &0xffe0) * (y_off0& 0xffe0) + ((y_off0& 0x01f)<<5) + 
                              (((image_w0 + x_off0 + parallax_para_r) & 0xffe0)<<5);
		de_scal_ch1r_offset = ((((2 * image_w1)<< 0x01)+0x1f)&0xffe0) * ((y_off1) & 0xffe0) + 
                              (((y_off1) & 0x01f)<<5) + ((((image_w1 + x_off1 + parallax_para_r)<<0x01) & 0xffe0)<<5);
		de_scal_ch2r_offset = 0x0;
