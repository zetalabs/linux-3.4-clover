...
#if 1
static void rtl8723au_module_power(int onoff)
{
	#if 1
    if (onoff)
        __gpio_set_value(gpio_wifi_power, 1);
    else
        __gpio_set_value(gpio_wifi_power, 0);
	#endif
}
#else
...
void rtl8723au_gpio_init(void)
{
	...
	gpio_wifi_power = val.gpio.gpio;
    if (0 != gpio_request_one(gpio_wifi_power, GPIOF_OUT_INIT_HIGH, NULL)){
